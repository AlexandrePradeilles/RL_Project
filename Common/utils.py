...
from Common.env import SuperMario
...
def make_handmade_mario(max_episode_steps, sticky_action=True, max_and_skip=True):
    env = SuperMario(size=10)
    env.max_episode_steps = max_episode_steps * 4
    if sticky_action:
        env = StickyActionEnv(env)
    # if max_and_skip:
    #     env = RepeatActionEnv(env)
