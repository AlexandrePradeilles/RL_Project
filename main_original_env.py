    ...
    if config["do_train"]:
            ...
            for t in tqdm(range(total_pre_normalization_steps)):
                ...
                for worker_id, env in enumerate(envs):
                    ...
                    if config["render"] and worker_id==0:
                        env.render()
                    env.t += 1
                    if env.t % env.spec.max_episode_steps == 0:
                        d = True
                    if info["flag_get"]:
                        r = 1
                    else:
                    ...
                    states.append(env.stacked_states[-1, ...].reshape(1, 84, 84))
        ...
        for iteration in tqdm(range(init_iteration + 1, config["total_rollouts_per_env"] + 1)):
            ...
            for t in range(config["rollout_length"]):
                ...
                infos = []
                for worker_id, env in enumerate(envs):
                    next_state, r, d, info = env.step(total_actions[worker_id, t])
                    if config["render"] and worker_id==0:
                        env.render()
                    env.t += 1
                    if env.t % env.spec.max_episode_steps == 0:
                    ...
                    if info["flag_get"]:
                        r = 0.2
                    elif env.max_pos // 100 < info["x_pos"] // 100:
                        r = 0.1
                    elif info["x_pos"] <= env.max_pos:
                        r = -0.01
                    else:
                        r = 0
                    env.max_pos = max(env.max_pos, info["x_pos"])
                    ...
                    next_states[worker_id] = env.stacked_states
                    total_next_obs[worker_id, t] = env.stacked_states[-1, ...]
                    if d:
                        state = env.reset()
                        env.stached_states = stack_states(env.stacked_states, state, True)
                        env.episode_reward = 0
